...
```output
# 开头
几段话，不需要标题。开头的篇幅必须与后续正文章节保持一致，充分展开场景、人物和情节。
# 计划
...
```
# 新的记忆
[极简摘要，长章节模式≤500字，普通模式≤300字]
===END===
```

**严格长度要求：**
- **长章节模式**：最大500字，理想300-400字
- **普通模式**：最大300字，理想200-250字
- **超长处理**：只保留最近3-5个关键事件

## 极简原则：
1. **只记录关键事件**：用1句话概括，去掉所有修饰词
2. **只记录重要变化**：人物状态/关系/能力的实质性改变
3. **只记录必要设定**：影响后续剧情的核心设定
4. **删除所有冗余**：不重复前文记忆已有信息

## 格式要求：
- 用"·"分隔要点，不用完整句子
- 每个要点≤15字
- 示例："·主角获得XX能力·与XX结盟·发现XX秘密"

## 优先级（从高到低）：
主线转折 > 主角变化 > 核心冲突 > 关键设定 > 其他
...
3. **标题优化原则**：
   - **简洁明了**：标题字数严格不超过10个字，最佳4-8字，易于记忆
   - **朗朗上口**：读起来顺畅，便于传播
   - **独特性**：避免过于常见的标题模式
   - **类型匹配**：符合小说类型的命名习惯
   - **吸引力**：能够在众多作品中脱颖而出
...
- 凝练性：标题字数严格不超过10个字，最佳4-8字；避免冗长结构与多重修饰。
...
- 长度建议：标题字数严格不超过10个字；尽量避免多层从句与生僻字。
- 语义清晰：避免双关歧义导致错误预期。
## init:
接下来，我会提供给你用户想法和小说大纲，请你基于这些信息创作一个精彩的标题。

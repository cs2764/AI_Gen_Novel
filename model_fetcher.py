...
class ModelFetcher:
    ...
    def fetch_models(self, provider: str, api_key: str, **kwargs) -> List[ModelInfo]:
        ...
        try:
            ...
            #     return self._fetch_zhipu_models(api_key, **kwargs)
                ...
                return self._fetch_lambda3_models(api_key, **kwargs)
            elif provider == 'grok':
    ...
    def _fetch_lambda3_models(self, api_key: str, base_url: str = "https://api.lambda.ai/v1") -> List[ModelInfo]:
        """获取Lambda3 (OpenAI兼容模式3) 模型列表"""
        headers = {
            'Authorization': f'Bearer {api_key}',
            'Content-Type': 'application/json'
        }
        ...
        try:
            ...
            for model_data in data.get('data', []):
                models.append(ModelInfo(
                    ...
                    created_at=str(model_data.get('created', '')),
                    owned_by=model_data.get('owned_by', 'lambda'),
                    description='Lambda3/OpenAI兼容模式3'
                ))
            ...
            if models:
                ...
                # 如果API返回空列表，使用默认模型列表
                return self._get_default_lambda3_models()
                
        except Exception as e:
            logger.warning(f"获取Lambda3模型列表失败: {e}，使用默认列表")
            return self._get_default_lambda3_models()
    
    def _get_default_lambda3_models(self) -> List[ModelInfo]:
        """获取Lambda3默认模型列表（当API请求失败时使用）"""
        known_models = [
            "llama-4-maverick-17b-128e-instruct-fp8",
            "llama-4-scout-17b-16e-instruct",
            "deepseek-r1-0528",

...
- 用户想法：用户的原始创意和特殊要求
- 人物列表：主要角色信息（如果已生成）
- 伏笔设定（可选）：已生成的伏笔/反转设定，规划章节时应合理安排伏笔的埋设与揭示时机
...
**3.5 时间线连贯性检查：**
   - **严格顺序原则**：章节规划必须按照时间顺序排列，第N章发生的事件必须在时间上晚于第N-1章
   - **因果链检查**：每个阶段的事件必须建立在前一阶段的基础上，不能出现"后面的章节描述了前面章节还没发生的事情的前因"
   - **禁止倒叙式章节**：不允许先写某事的结果/后果，再在后续章节中写这件事发生的过程
   - **转场顺序**：如果有多条剧情线并行，应在同一章或相邻章节内处理同一时间段的事件，而不是先走完A线的未来再回到B线的过去
...
6. **时间线连贯性**：章节规划必须严格按照时间顺序排列，后一章的事件必须在时间上晚于前一章，绝不允许出现"第N章讲了某事的结果，第N+1章却讲这件事发生前的过程"的情况
...
- ✓ 是否有足够的冲突和张力
- ✓ 是否保持了简洁性
- ✓ 时间线是否严格按顺序推进，无倒序或回溯式章节排列
- ✓ 因果关系是否正确（前一章的事件是后一章的前提，而非相反）
- ✓ 多线并行时，同一时间段的事件是否在相邻章节内处理

...
1. **深入理解内容**：
   ...
   - 把握故事的类型和风格
   - 找出最吸引人的卖点
   - **识别改编来源**：如果用户想法中提到是改编、重写、致敬、二创某部作品，需记住原作标题
...
2. **标题创作策略**：
   ...
   - **悬念设置法**：
     ...
     * 例：《龙族》《盗墓笔记》
   - **改编致敬法**：
     * 当用户想法明确提到改编或基于某部作品时，标题中应体现原作元素
     * 可在标题中融入原作名称、角色名或标志性元素
     * 让读者一眼看出与原作的关联
     * 例：基于《西游记》改编 → 《大圣归来》；基于某作品改写 → 在标题中保留原作关键词

3. **标题优化原则**：
   - **简洁明了**：
     * 标题字数严格不超过10个字，最佳长度4-8字
     * 避免冗长复杂的表述
     * 每个字都要有价值
   - **朗朗上口**：
     * 读起来顺畅，便于传播
...
- **动词驱动**：使用有力的动词增强动感
- **意象结合**：将抽象概念与具体意象结合
- **改编关联**：如果用户想法中明确提到改编/改写/重写某部作品，标题中应包含或融入原作标题的关键元素，让读者能识别出与原作的关系
